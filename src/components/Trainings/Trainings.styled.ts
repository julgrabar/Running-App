...
export const Heading = styled.span<{ dark?: boolean }>`
  ...
  color: ${({ theme, dark }) =>
    dark ? theme.colors.secondaryFontColor : theme.colors.mainFontColor};
  font-weight: 700;
  font-size: 32px;
  margin-bottom: 30px;
`;

export const TrainingItem = styled.li<{ passed?: boolean }>`
  border-radius: 20px;
  padding: 20px;
  cursor: pointer;
  text-align: left;
  background: url(${arrow}) center right/50px 50px no-repeat,
    linear-gradient(
      274.42deg,
      rgba(146, 163, 253, 20%) 0%,
      rgba(157, 206, 255, 20%) 124.45%
    );
