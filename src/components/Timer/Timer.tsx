...
export const Timer = ({exersice, onTimerFinished}: {exersice: IExercise | IWarming, onTimerFinished: ()=>void}) => {
    ...
    return <TimerWrapper>
    <span>
        {Math.floor(seconds/60) > 0 && `${Math.floor(seconds/60)} :`}{(seconds%60).toString().padStart(2,"0")}
    </span>
    <Button onClick={isActive ? stopTimer : startTimer} primary round={{diameter: 50}}>{isActive ? <img src={stopImg} alt="Stop" /> : <img src={startImg} alt="Start" />}</Button>
    </TimerWrapper>
}